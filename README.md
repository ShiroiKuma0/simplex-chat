...
**SimpleX Chat for Android that plays nice with dictionary keyboards — and lets you style your delivery ticks.**

A fork of [SimpleX Chat](https://github.com/simplex-chat/simplex-chat) with **major additions**: an IME-commit-race fix that makes dictionary keyboards (Multiling O, Czech autocorrect, CJK input) work in the message field, fully customizable delivery ticks (size, thickness, colors), and a black-yellow fork identity.

Installs **side-by-side** with official SimpleX Chat (app id `shiroikuma.simplex`).

**📥 Latest release: [`7.0-beta.2+1`](https://github.com/ShiroiKuma0/simplex-chat/releases/latest)** — [all releases & APK downloads »](https://github.com/ShiroiKuma0/simplex-chat/releases)
...
Appearance → Message ticks: scale the sent/received checkmarks from 1× to 15×, set their stroke thickness, and give sent and received ticks their own colors (defaults: light-blue sent, theme-yellow received). The ticks are redrawn as true vector strokes on a Canvas, so they stay crisp at any size instead of plateauing at the stock icon's intrinsic limit.
...
The fork rebases onto every new upstream release — **betas included** — usually days after it ships. APKs are `arm64-v8a` only and versioned `<upstream>+<build>` (e.g. `7.0-beta.2+1`), with version codes that always sort above the corresponding upstream build.
