...
**The assemble task is `:android:assembleFossRelease`** (from upstream `v7.1.0-beta.0` onward), and the unsigned APK lands under `build/outputs/apk/**foss**/release/`. Plain `:android:assembleRelease` now hard-fails on upstream's own task-graph guard, because it drags in the Play-Billing `google` flavor. See simplex-chat-build → **"Product flavors"** for the full story and the path table.
